...
G4VPhysicalVolume* DetectorConstruction::Construct(){
  ...
  */
  ...
  G4Material* XenonArgon=NULL;

  G4double densityXe (1.4*mg/cm3 * gasPressure/bar);
  G4double densityAr (5.0*mg/cm3 * gasPressure/bar);
  G4double density;
  density = fGasModelParameters->GetFracXe() * densityXe + (1. - fGasModelParameters->GetFracXe()) * densityAr;
  XenonArgon = new G4Material("XenonArgon", density, 2);
  ...
  XenonArgon->AddMaterial(Xenon,fGasModelParameters->GetFracXe());
  XenonArgon->AddMaterial(Argon,1.0-fGasModelParameters->GetFracXe());
  ...
  XenonArgon->SetMaterialPropertiesTable(mptXenon);
  ...
  */
  ...
  logicGasBox = new G4LogicalVolume(detectorSolid,XenonArgon,"detectorLogical");
