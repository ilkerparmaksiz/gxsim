...
const static G4double fieldLEM   = 3000.0; // V/cm higher than 3k (as used for 2 bar) for 10 bar!
...
GarfieldVUVPhotonModel::GarfieldVUVPhotonModel(GasModelParameters* gmp, G4String modelName,G4Region* envelope,DetectorConstruction* dc,GasBoxSD* sd) :
	...
	fgasFile = gmp->GetGasFile();
	ffracXe = gmp->GetFracXe();
	InitialisePhysics();
...
void GarfieldVUVPhotonModel::InitialisePhysics(){
	...
	fMediumMagboltz->SetComposition("Xe", 100.*ffracXe, "Ar", 100.0*(1.0-ffracXe));
	fMediumMagboltz->Initialise(true);
	fMediumMagboltz->DisableDebugging();
	gasFile =  "Xenon.gas"; // we will write over this with value from .mac file
	if (fgasFile != "" && ffracXe>0.)
	  {
	    gasFile = fgasFile + "_" + std::to_string(ffracXe) + ".gas";
	  }
	else
	  {
	    gasFile = "Xenon_badspec_mix.gas";
	  }
	...
	if(gasFile!=""){
	  ...
	  fMediumMagboltz->SetFieldGrid(10.,60000.,20,useLog); // In 99:1 N2:O2 E/p = 3000000/3 causes Magboltz to crash. EC, 5-Nov-2021.
	  // E = 80000 causes crash. EC, 4-Nov-2022.`
	  fMediumMagboltz->GenerateGasTable(5); // num of 1E7 collisions to do ... something.
	  std::cout << "Writing gasfile: " << gasFile << std::endl;
	   fMediumMagboltz->WriteGasFile(gasFile.c_str());      
	   std::cout << "Wrote gasfile: " << gasFile << std::endl;
